...
function filter_section(id) {
  ...
  if (id != "All") {
    ...
    $(
      ".col-md-4.col-sm-6.col-xs-6[data-group='" +
        section_groups[id]["group_name"] +
        "']"
    ).show();
  } else {
...
function show_element_style(id) {
  frappe.call({
    method:
      "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_element_properties",
    args: { id: id },
    async: false,
    callback: function (r) {
      if (r.message.css_properties_list) {
        ...
        elementstyledialog.set_primary_action(__("Save"), function () {
          // cap nhan media
          if (
            "pc-font-size" in fonts_media_query_data &&
            "tab-font-size" in fonts_media_query_data
          ) {
            let temp = fonts_media_query_data["pc-font-size"];
            delete fonts_media_query_data["pc-font-size"];
            fonts_media_query_data["pc-font-size"] = temp;
          }
          ...
          if (popup_variable_color_codes.hasOwnProperty("bg-divider-img")) {
            ...
            gra_patt_mask["bg-divider-img"]
              ? (style_json["bg-divider-img"] = gra_patt_mask["bg-divider-img"])
              : "";
            gra_patt_mask["bg-divider-img"]
              ? (divider_css_design +=
                  '-webkit-mask:url("' +
        ...
        for (var i = 0; i < units_sections.length; i++) {
          ...
          elementstyledialog.$wrapper
            ...
            .find("select")
            .val(
              units_sections_field_with_units[i] &&
                units_sections_field_with_units[i].unit
                ? units_sections_field_with_units[i].unit
                : "px"
            );
        }
        ...
        if (enable_font_media_query) {
          ...
          $(
            elementstyledialog.$wrapper.find(
              '[data-fieldname="font-size"] input'
            )
          ).on("keyup", (e) => {
