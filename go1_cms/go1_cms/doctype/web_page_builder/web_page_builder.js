...
frappe.ui.form.on("Web Page Builder", {
  refresh: function (frm) {
    ...
    if (frm.doc.document && frm.doc.w_page_type == "List") {
      frm.set_query("data_fetch_from", function () {
        return {
          query:
            "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_linked_docs",
          filters: {
      ...
      if (frm.doc.w_page_type == "List" && frm.doc.enable_side_menu == 1) {
        frappe.call({
          method:
            "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_side_menu_fields",
          args: {
    ...
    if (frm.doc.document && frm.doc.w_page_type == "Detail") {
      frappe.call({
        method:
          "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_linked_fields",
        args: {
          document: frm.doc.document,
        },
        freeze: true,
        callback: function (r) {
          ...
          for (var i = 0; i < r.message.length; i++) {
            options.push({
              value: r.message[i].fieldname,
              label: r.message[i].options,
            });
          }
          frm.set_df_property(
    ...
    if (frm.doc.__islocal) {
      frappe.db.get_list("Page Template", {}).then((res) => {
        if (
          res.length > 0 &&
          (frm.doc.page_type == "Responsive" || frm.doc.page_type == "Adaptive")
        ) {
          ...
          $('button[data-label="Pick%20From%20Template"]').attr(
            "class",
            "btn btn-xs btn-default"
          );
          $('button[data-label="Pick%20From%20Template"]').html(
            "<i class='fa fa-check' style='margin-right:5px'></i> Pick From Template"
          );
        }
      });
    }
    if (!frm.__islocal) {
      ...
      if (
        frm.doc.w_page_type == "List" &&
        frm.doc.list_style_image != undefined
      ) {
        $("#list_style_image").remove();
        var img_html =
          '<img src="' +
          frm.doc.list_style_image +
          '" style="margin-top: 10px;" id="list_style_image"/>';
        $('button[data-fieldname="choose_list_style"]')
          .parent()
          .append(img_html);
      }
      if (
        frm.doc.w_page_type == "Detail" &&
        frm.doc.detail_layout_image != undefined
      ) {
        $("#detail_style_image").remove();
        var img_html =
          '<img src="' +
          frm.doc.detail_layout_image +
          '" style="margin-top: 10px;" id="detail_style_image"/>';
        $('button[data-fieldname="choose_detail_style"]')
          .parent()
          .append(img_html);
      }
    }
    if (!frm.__islocal && frm.doc.published == 1) {
      ...
      $('button[data-label="Preview"]').html(
        "<i class='fa fa-eye' style='margin-right:5px'></i> Preview"
      );
    }
    if (
      frm.doc.route &&
    ...
    if (!frm.doc.__islocal && frm.doc.is_converted_to_template == 0) {
      frm.add_custom_button(__("Save As Template"), function () {
        ...
        title_dialog.set_primary_action(__("Save As Template"), function () {
          ...
          frappe.call({
            method:
              "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.save_as_template",
            args: {
            ...
            callback: function (r) {
              ...
              frappe.msgprint(
                __("Page has been saved as Template successfully")
              );
      ...
      $('button[data-label="Save%20As%20Template"]').attr(
        "class",
        "btn btn-xs btn-default"
      );
      $('button[data-label="Save%20As%20Template"]').html(
        "<i class='fa fa-save' style='margin-right:5px'></i> Save As Template"
      );
    }
  },
  store_global_font_and_weight_values(frm) {
    // console.log(">>triger<<");
    frappe.db.get_value("Web Theme", { is_active: 1 }, "name").then((r) => {
  ...
  edit_with_go1_builder: function (frm) {
    window.location.href = "/app/go1cms-builder?route=" + frm.doc.route;
  },
  ...
  section_html: function (frm, field, html, device_type) {
    ...
    if (frm.doc[field] && frm.doc[field].length > 0) {
      frm.doc[field].map((f) => {
        ...
        if (f.section_type != "Slider") allow = true;
          ...
          let slider_link = $(
            `<button class="btn btn-primary btn-xs" style="margin-right: 10px;"><span class="fa fa-edit" style="margin-right:5px"></span>Edit Data</button>`
          );
        ...
        if (f.allow_update_to_style == 1) {
          let style_section = $(
            `<button class="btn btn-warning btn-xs" style="margin-right: 10px;"><span class="fa fa-code" style="margin-right:5px"></span>Edit Styles</button>`
          );
        ...
        let delete_section = $(
          `<button class="btn btn-danger btn-xs"><span class="fa fa-trash" style="margin-right:5px"></span>Delete</button>`
        );
    ...
    let create_section = $(
      `<button class="btn btn-sm btn-primary" style="margin-bottom: 10px;">Add New Section</button>`
    ).appendTo(wrapper);
    // add_section.click(()=>{
    //     new add_new_section({
    //         parentfield: field,
  ...
  delete_sections: function (frm, name, parentfield) {
    frappe.confirm("Are you sure to want to remove this section?", function () {
      frappe.call({
        method:
  ...
  edit_styles_sections: function (frm, name, parentfield) {
    ...
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_section_properties",
      ...
      callback: function (r) {
        if (r.message.styles) {
          ...
          for (var i = 0; i < filelist.length; i++) {
            if (filelist[i].child_properties) {
              ...
              for (var k = 0; k < child_properties.length; k++) {
                ...
                for (const [key, value] of Object.entries(json_data)) {
                  if (key == child_properties[k].fieldname) {
                    // console.log(key, value, child_properties[k].fieldname);
                    // child_properties[k].default=value

                    // Modify by gopi on 17/9/22
                    if (
                      !child_properties[k].allow_media_query &&
                      key != "font-size" &&
                      value &&
                      (value.toString().endsWith("-px") ||
                        value.toString().endsWith("-%") ||
                        value.toString().endsWith("-em") ||
                        value.toString().endsWith("-rem"))
                    ) {
                      child_properties[k].default = value.split("-")[0];

                      if (child_properties[k].allow_units) {
                        if (child_properties[k].allow_units == 1) {
                          units_sections_field_with_units.push({
                            field_name: child_properties[k].fieldname,
                            unit: value.split("-")[1],
                          });
                        }
                      }
                    } else if (
                      (key == child_properties[k].fieldname &&
                        key == "bg-gradient-color") ||
                      (key == child_properties[k].fieldname &&
                        key == "bg-pattern-color") ||
                      (key == child_properties[k].fieldname &&
                        key == "bg-mask-color") ||
                      (key == child_properties[k].fieldname &&
                        key == "divider-color") ||
                      ...
                      if (
                        child_properties[k].fieldname == "bg-gradient-color"
                      ) {
                        // console.log(">> gradiant value <<",value)
                        gra_patt_mask["bg-gradient-color"] = value;
                      } else if (
                        child_properties[k].fieldname == "bg-pattern-color"
                      ) {
                        gra_patt_mask["bg-pattern-color"] = value;
                      } else if (
                        child_properties[k].fieldname == "bg-mask-color"
                      ) {
                        gra_patt_mask["bg-mask-color"] = value;
                      } else if (
                        child_properties[k].fieldname == "divider-color"
                      ) {
                        gra_patt_mask["divider-color"] = value;
                        child_properties[k].default = value;
                      } else if (
                        child_properties[k].fieldname == "bg-divider-img"
                      ) {
                      ...
                      if (typeof value === "object") {
                        Object.keys(value).map((each) => {
                          if (each == "mbl-font-size") {
                            fonts_media_query_data["mbl-font-size"] =
                              value["mbl-font-size"];
                          } else if (each == "tab-font-size") {
                            fonts_media_query_data["tab-font-size"] =
                              value["tab-font-size"];
                          } else if (each == "pc-font-size") {
                            fonts_media_query_data["pc-font-size"] =
                              value["pc-font-size"];
                            child_properties[k].default =
                              value["pc-font-size"].split("-")[0];
                            if (child_properties[k].allow_units) {
                              if (child_properties[k].allow_units == 1) {
                                units_sections_field_with_units.push({
                                  field_name: child_properties[k].fieldname,
                                  unit: value["pc-font-size"].split("-")[1],
                                });
                              }
                            }
                          }
                ...
                if (child_properties[k].fieldname != "font-family") {
                  ...
                  if (
                    child_properties[k].fieldname == "font-size" &&
                    child_properties[k].allow_media_query
                  ) {
                    enable_font_media_query = true;
                  }
                  ...
                  if (
                    ...
                    if (filelist[i].allow_units) {
                      if (filelist[i].allow_units == 1) {
                        units_sections_field_with_units.push({
                          field_name: filelist[i].fieldname,
                          unit: value.split("-")[1],
                        });
                      }
                    }
                  } else if (
                    (key == filelist[i].fieldname &&
                      key == "bg-gradient-color") ||
                    (key == filelist[i].fieldname &&
                      key == "bg-pattern-color") ||
                    (key == filelist[i].fieldname && key == "bg-mask-color") ||
                    (key == filelist[i].fieldname && key == "divider-color") ||
                    (key == filelist[i].fieldname && key == "bg-divider-img")
                  ) {
              ...
              if (filelist[i].fieldname != "font-family") {
                ...
                if (
                  filelist[i].fieldname == "font-size" &&
                  filelist[i].allow_media_query
                ) {
                  enable_font_media_query = true;
                }
                                                    ...
                                                    <div class="gradient-color" style="${
                                                      gra_patt_mask[
                                                        ...
                                                        ? "background:" +
                                                          ...
                                                          ";"
                                                        : ""
                                                    }float:left;margin-right: 10px;width:35px;height:26px;border-radius:7px;margin-top:10px;"></div>
                                                </div>
                                                </div>`;
          $(gradient_html).insertBefore(
            styledialog.$wrapper.find(
              '[data-fieldname="bg-gradient-color"] button'
            )
          );
          ...
          $(mask_html).insertBefore(
            styledialog.$wrapper.find('[data-fieldname="bg-mask-color"] button')
          );
          ...
          $(pattern_html).insertBefore(
            styledialog.$wrapper.find(
              '[data-fieldname="bg-pattern-color"] button'
            )
          );
          ...
          $(divider_html).insertBefore(
            styledialog.$wrapper.find(
              '[data-fieldname="bg-divider-img"] button'
            )
          );

          styledialog.$wrapper
            .find('[data-fieldname="bg-gradient-color"] button')
            .on("click", (e) => {
              // console.log(">> gradinat color working<<")
              poup_bg_dialog("Gradient");
            });
          styledialog.$wrapper
            .find('[data-fieldname="bg-pattern-color"] button')
            .on("click", (e) => {
              // console.log(">> pattern color working <<")
              poup_bg_dialog("Pattern");
            });
          styledialog.$wrapper
            .find('[data-fieldname="bg-mask-color"] button')
            .on("click", (e) => {
          ...
          styledialog.$wrapper
            .find('[data-fieldname="bg-divider-img"] button')
            .on("click", (e) => {
              // console.log(">> mask color working <<")
          ...
          styledialog.set_primary_action(__("Save"), function () {
            ...
            if (enable_font_media_query) {
              ...
              Object.keys(fonts_media_query_data).map((e_res) => {
                if (e_res == "mbl-font-size") {
                  style_json["font-size"]["mbl-font-size"] =
                    fonts_media_query_data[e_res];
                } else if (e_res == "tab-font-size") {
                  style_json["font-size"]["tab-font-size"] =
                    fonts_media_query_data[e_res];
                } else if (e_res == "pc-font-size") {
                  style_json["font-size"]["pc-font-size"] =
                    fonts_media_query_data[e_res];
                }
              });
            }
            ...
            for (let k in values) {
              ...
              if (
                ...
                // modify by gopi for dividers 19/9/22
                  ...
                  divider_css_design +=
                    "width:" + values[k] + units + " !important;";
                  ...
                  divider_css_design +=
                    "height:" + values[k] + units + " !important;";
              ...
              if (k == "font-size" && enable_font_media_query) {
                Object.keys(fonts_media_query_data).map((e_res) => {
                  if (e_res == "pc-font-size") {
                    font_size_css_design += `@media (min-width: 992px){.${
            ...
            if (
              popup_variable_color_codes.hasOwnProperty("bg-gradient-color")
            ) {
              popup_variable_color_codes["bg-gradient-color"]
                ? (style_json["bg-gradient-color"] =
                    popup_variable_color_codes["bg-gradient-color"])
                : "";
              popup_variable_color_codes["bg-gradient-color"]
                ? (css_design +=
                    "background:" +
                    popup_variable_color_codes["bg-gradient-color"].split(
                      ";"
                    )[0] +
                    " !important;")
                : "";
            } else if (gra_patt_mask.hasOwnProperty("bg-gradient-color")) {
              gra_patt_mask["bg-gradient-color"]
                ? (style_json["bg-gradient-color"] =
                    gra_patt_mask["bg-gradient-color"])
                : "";
              gra_patt_mask["bg-gradient-color"]
                ? (css_design +=
                    "background:" +
                    gra_patt_mask["bg-gradient-color"].split(";")[0] +
                    " !important;")
                : "";
            }

            if (popup_variable_color_codes.hasOwnProperty("bg-pattern-color")) {
              popup_variable_color_codes["bg-pattern-color"]
                ? (style_json["bg-pattern-color"] =
                    popup_variable_color_codes["bg-pattern-color"])
                : "";
              popup_variable_color_codes["bg-pattern-color"]
                ? (css_design +=
              ...
              gra_patt_mask["bg-pattern-color"]
                ? (style_json["bg-pattern-color"] =
                    gra_patt_mask["bg-pattern-color"])
                : "";
              gra_patt_mask["bg-pattern-color"]
            ...
            if (popup_variable_color_codes.hasOwnProperty("bg-mask-color")) {
              popup_variable_color_codes["bg-mask-color"]
                ? (style_json["bg-mask-color"] =
                    popup_variable_color_codes["bg-mask-color"])
                : "";
              popup_variable_color_codes["bg-mask-color"]
                ? (css_design +=
              ...
              gra_patt_mask["bg-mask-color"]
                ? (style_json["bg-mask-color"] = gra_patt_mask["bg-mask-color"])
                : "";
              gra_patt_mask["bg-mask-color"]
                ? (css_design +=
                    'background-image:url("' +
            ...
            if (popup_variable_color_codes.hasOwnProperty("bg-divider-img")) {
              popup_variable_color_codes["bg-divider-img"]
                ? (style_json["bg-divider-img"] =
                    popup_variable_color_codes["bg-divider-img"])
                : "";
              popup_variable_color_codes["bg-divider-img"]
                ? (divider_css_design +=
            ...
            frappe.call({
              method:
                "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.update_section_properties",
              args: {
          ...
          for (var i = 0; i < units_sections.length; i++) {
            ...
            select_html +=
              '<div class="select-icon " style="padding-left: inherit;padding-right: inherit;position: absolute;pointer-events: none;top: 7px;right: 6px;"><svg class="icon  icon-sm" style=""><use class="" href="#icon-select"></use></svg></div>';
            styledialog.$wrapper
              .find(".form-control[data-fieldname='" + units_sections[i] + "']")
              .parent()
              .append(select_html);

            // kiem tra xem co unit khong neu co thi gan gia tri
            // neu khong thi mac dinh la 'px'
            let unit = "px";
            let find_unit = units_sections_field_with_units.find(
              (x) => x.field_name === units_sections[i]
            );
            if (find_unit && find_unit.unit) {
              unit = find_unit.unit;
            }
          ...
          if (enable_font_media_query) {
            styledialog.$wrapper
              .find('[data-fieldname="font-size"] label')
              .parent()
              .css({ display: "flex", gap: "10px", "align-items": "center" });
            styledialog.$wrapper
              .find('[data-fieldname="font-size"] label')
              .parent()
              .css({ display: "flex", gap: "10px", "align-items": "center" });
            styledialog.$wrapper
              .find('[data-fieldname="font-size"] label')
              .parent()
              .append(`<i id="mbl-font-icon" style="font-size:20px;cursor:pointer;padding-bottom:3px;" class="fa fa-mobile" aria-hidden="true"></i>
            ...
            styledialog.$wrapper
              .find('[data-fieldname="font-size"] input')
              .attr("input_value", "Web");
            styledialog.$wrapper
              .find('[data-fieldname="font-size"] #mbl-font-icon')
              .on("click", () => {
                // console.log(">>mbl working<<")
                styledialog.$wrapper
                  .find('[data-fieldname="font-size"] input')
                  .attr("input_value", "Mobile");
                if (fonts_media_query_data["mbl-font-size"]) {
                  styledialog.$wrapper
                    .find('[data-fieldname="font-size"] input')
                    .val(fonts_media_query_data["mbl-font-size"].split("-")[0]);
                  styledialog.$wrapper
                    .find(".form-control[data-fieldname='font-size']")
                    .parent()
                    .find("select")
                    .val(
                      fonts_media_query_data["mbl-font-size"].split("-")[1]
                        ? fonts_media_query_data["mbl-font-size"].split("-")[1]
                        : "px"
                    );
                } else {
                ...
                styledialog.$wrapper
                  ...
                  .attr(
                    "style",
                    "font-size:22px;cursor:pointer;font-weight: 700;color: #0101ff;"
                  );
                styledialog.$wrapper
                  .find('[data-fieldname="font-size"] #pc-font-icon')
                  .attr("style", "font-size:15px;cursor:pointer;");
                styledialog.$wrapper
                  .find('[data-fieldname="font-size"] #tab-font-icon')
                  .attr("style", "font-size:15px;cursor:pointer;");
              });
            styledialog.$wrapper
              .find('[data-fieldname="font-size"] #tab-font-icon')
              .on("click", () => {
                ...
                styledialog.$wrapper
                  .find('[data-fieldname="font-size"] input')
                  .attr("input_value", "Tab");
                if (fonts_media_query_data["tab-font-size"]) {
                  styledialog.$wrapper
                    .find('[data-fieldname="font-size"] input')
                    .val(fonts_media_query_data["tab-font-size"].split("-")[0]);
                  styledialog.$wrapper
                    .find(".form-control[data-fieldname='font-size']")
                    .parent()
                    .find("select")
                    .val(
                      fonts_media_query_data["tab-font-size"].split("-")[1]
                        ? fonts_media_query_data["tab-font-size"].split("-")[1]
                        : "px"
                    );
                } else {
                ...
                styledialog.$wrapper
                  ...
                  .attr(
                    "style",
                    "font-size:17px;cursor:pointer;font-weight: 700;color: #0101ff;"
                  );
                styledialog.$wrapper
                  .find('[data-fieldname="font-size"] #pc-font-icon')
                  .attr("style", "font-size:15px;cursor:pointer;");
                styledialog.$wrapper
                  .find('[data-fieldname="font-size"] #mbl-font-icon')
                  .attr("style", "font-size:20px;cursor:pointer;");
              });
            styledialog.$wrapper
              .find('[data-fieldname="font-size"] #pc-font-icon')
              .on("click", () => {
                ...
                if (fonts_media_query_data["pc-font-size"]) {
                  styledialog.$wrapper
                    .find('[data-fieldname="font-size"] input')
                    .val(fonts_media_query_data["pc-font-size"].split("-")[0]);
                  styledialog.$wrapper
                    .find(".form-control[data-fieldname='font-size']")
                    .parent()
                    .find("select")
                    .val(
                      fonts_media_query_data["pc-font-size"].split("-")[1]
                        ? fonts_media_query_data["pc-font-size"].split("-")[1]
                        : "px"
                    );
                } else {
            ...
            $(
              styledialog.$wrapper.find('[data-fieldname="font-size"] input')
            ).on("keyup", (e) => {
              let entered_value = $(e.target).val();
              let unit = $(
                styledialog.$wrapper.find('[data-fieldname="font-size"] select')
              ).val();
              let input_value = $(
                styledialog.$wrapper.find('[data-fieldname="font-size"] input')
              ).attr("input_value");
            ...
            $(
              ...
              if (input_value == "Web") {
                if (fonts_media_query_data["pc-font-size"]) {
                  let each_val =
                    fonts_media_query_data["pc-font-size"].split("-");
                  fonts_media_query_data["pc-font-size"] =
                    each_val[0] + "-" + unit;
                }
              } else if (input_value == "Tab") {
                if (fonts_media_query_data["tab-font-size"]) {
                  let each_val =
                    fonts_media_query_data["tab-font-size"].split("-");
                  fonts_media_query_data["tab-font-size"] =
                    each_val[0] + "-" + unit;
                }
              } else if (input_value == "Mobile") {
                if (fonts_media_query_data["mbl-font-size"]) {
                  let each_val =
                    fonts_media_query_data["mbl-font-size"].split("-");
                  fonts_media_query_data["mbl-font-size"] =
                    each_val[0] + "-" + unit;
                }
              }
          ...
          function poup_bg_dialog(bg_type) {
            frappe.call({
              method:
                "go1_cms.go1_cms.doctype.web_theme.web_theme.get_color_code_data",
              ...
              callback: (res) => {
                ...
                if (res && res.message && res.message.length > 0) {
                  ...
                  frappe.msgprint(
                    `There is no <b>${bg_type}</b> records found..!`
                  );
                }
              },
            });
          ...
          function generate_colors_html(gradiant_data, bg_type) {
            ...
            gradiant_data.map((each_gradiant) => {
              ...
              if (
                ...
                !each_gradiant.css_properties.startsWith("data") &&
                !each_gradiant.css_properties.startsWith("<svg") &&
                !each_gradiant.css_properties.startsWith("/files")
              ) {
                temp_html =
                  $(`<div class="each-bg-div" value="${each_gradiant.css_properties}" bg_type="${bg_type}" style="flex: 0 0 calc(20% - 20px);cursor:pointer;">
                ...
                temp_html = $(`<div class="each-bg-div" value="${
                  each_gradiant.css_properties
                ...
                temp_html =
                  $(`<div class="each-bg-div" bg_type="${bg_type}" value="${
                    each_gradiant.css_properties
                ...
                temp_html =
                  $(`<div class="each-bg-div" bg_type="${bg_type}" value="${dividers_values.indexOf(
                    each_gradiant.css_properties
                  )}" style="flex: 0 0 calc(20% - 20px);cursor:pointer;">
              ...
              if (temp_html) {
                temp_html.click((e) => {
                  ...
                  if ($(e.target).attr("bg_type") == "Gradient") {
                    ...
                    styledialog.$wrapper
                      .find(
                        '[data-fieldname="bg-gradient-color"] .gradient_modify_sec .btn-previw-section .gradient-color'
                      )
                      .css({
                        background: `${
                          $(e.target).attr("value").split(";")[0]
                        }`,
                      });
                    bg_dialog.hide();
                    ...
                    styledialog.$wrapper
                      .find(
                        '[data-fieldname="bg-pattern-color"] .pattern_modify_sec .btn-previw-section .pattern-color'
                      )
                      .css({ background: `${$(e.target).attr("value")}` });
                    bg_dialog.hide();
                    ...
                    styledialog.$wrapper
                      .find(
                        '[data-fieldname="bg-mask-color"] .mask_modify_sec .btn-previw-section .mask-color'
                      )
                      .css({ background: `${$(e.target).attr("value")}` });
                    bg_dialog.hide();
                    ...
                    let divider_background = gra_patt_mask["divider-color"]
                      ? gra_patt_mask["divider-color"]
                      : "Red";
                    styledialog.$wrapper
                      .find(
                        '[data-fieldname="bg-divider-img"] .divider_modify_sec .btn-previw-section .divider-color'
                      )
                      .css({
                        "-webkit-mask": `url('${$(e.target).attr(
                          "value"
                        )}')no-repeat center / contain`,
                ...
                $(bg_wrapper)
                  .find(
                    `#${cur_frm.r___id}[bg_type="${bg_type}"].bg-main-div .bg-sub-div`
                  )
                  .append(temp_html);
              }
            });
            $(bg_wrapper)
              .find(
                `#${cur_frm.r___id}[bg_type="${bg_type}"].bg-main-div .bg-sub-div svg`
              )
...
var add_new_section = Class.extend({
  ...
  get_section_templates: function () {
    ...
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_section_templates",
      args: {
        device_type: me.device_type,
      },
      async: false,
  ...
  make_dialog: function () {
    ...
    if (this.section_type == "new") title = "Choose Section Template";
  ...
  section_html: function () {
    ...
    if (me.groups.length > 0) {
      wrapper
        .find("#SectionList")
        .append(
          "<div class='col-md-3' id='sec-left-content' style='background-color: #fff;    height: 478px;margin-left: -20px;position: absolute;left: 0;border-right: 1px solid #ddd;overflow-y: auto;overflow-x: hidden;margin-top: -15px;width: 25%;'></div>"
        );
      wrapper
        .find("#SectionList")
        .append(
          "<div class='col-md-9' id='sec-right-content' style='padding-top:20px;margin-left:25%;max-height: 500px;overflow-y: scroll;'></div>"
        );
      var group_html =
      ...
      for (var i = 0; i < me.groups.length; i++) {
        ...
        group_html +=
          ...
          ")>" +
          me.groups[i].group_name +
          ' <i class="fa fa-chevron-right" style="float: right;padding-top: 2px;font-size: 13px;color: #9e9e9e;font-weight: normal;"></i></a></li>';
      }
      group_html += "</ul>";
    ...
    data.map((f) => {
      ...
      let item =
        $(`<div class="col-md-4 col-sm-6 col-xs-6" style="float:left" data-group='${f.section_group}'>
      ...
      item.click(function () {
        if (me.section_type == "existing") {
          ...
          if (f.image) {
            image_html =
              "<label class='control-label' style='padding-right: 0px;'>Section Preview</label><img src='" +
              f.image +
              "' style='border: 1px solid #ddd;padding: 5px;border-radius: 5px;' />";
            title_fileds.push({
              fieldname: "section_image",
              fieldtype: "HTML",
              label: __("Section Privew"),
            });
          }
          var title_dialog = new frappe.ui.Dialog({
...
function bind_layout_data(wrapper) {
  ...
  function bind_layout_data(layout_data) {
    layout_data.map((each_p) => {
      ...
      let item =
        $(`<div class="col-md-4 col-sm-6 col-xs-6" style="float:left" data-group='layout-section'>
                            ${p_html}
                    </div>`);
      wrapper.find("#SectionList").find("#sec-right-content").append(item);
      item.click(() => {
...
var modify_section_data = Class.extend({
  ...
  save_data: function () {
    ...
    //     frappe.call({
    //         method: 'go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.generate_css_file',
    //         args: {
...
var add_page_template = Class.extend({
  ...
  get_page_templates: function () {
    ...
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_page_templates",
      args: {},
      async: false,
      callback: function (r) {
        console.log(r);
  ...
  make_dialog: function () {
    ...
    this.dialog.$wrapper
      ...
      .attr(
        "style",
        "width: calc(100%);margin-left: 0px;padding-top: 0;margin-top: 0;"
      );
    this.dialog.$wrapper
      .find('[data-fieldname="section_html"]')
      .attr("style", "margin-bottom: 0;");
  ...
  section_html: function () {
    ...
    wrapper
      .find("#PageList")
      .append(
        "<div class='col-md-12' id='sec-right-content' style='padding-top: 15px;height: 510px;overflow-y: auto;'></div>"
      );
    data.map((f) => {
      ...
      item.click(function () {
        if (
          cur_frm.doc.page_title != "" &&
          cur_frm.doc.page_title != undefined
        ) {
          me.selected_section = { template: f.name };
          $(html).find(".section-item").removeClass("active");
          $(item).find(".section-item").addClass("active");
          frappe.confirm(
            ...
            () => {
              frappe.call({
                method:
                  "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.import_sections_from_template",
                args: {
                ...
                callback: function (r) {
                  if (r.message) {
                    ...
                    for (var i = 0; i < mobile_sections.length; i++) {
                      ...
                      a.allow_update_to_style =
                        mobile_sections[i].allow_update_to_style;
                    ...
                    for (var i = 0; i < web_sections.length; i++) {
                      ...
                      a.allow_update_to_style =
                        web_sections[i].allow_update_to_style;
...
var add_list_template = Class.extend({
  ...
  get_page_templates: function () {
    ...
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_list_page_templates",
      args: {},
      async: false,
      callback: function (r) {
...
var columns_mapping = Class.extend({
  ...
  get_section_columns: function () {
    ...
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_section_columns",
      args: {
  ...
  section_html: function () {
    ...
    let wrapper = this.dialog.fields_dict.section_html.$wrapper.empty();
    let html = $(
      `<div class="row" id="ColumnList" style="padding: 0 15px;"></div>`
    ).appendTo(wrapper);
    ...
    wrapper
      .find("#ColumnList")
      .append(
        "<table style='margin: 5px 0;' class='table table-bordered' ><thead><tr><th>List Column</th><th>Column Map</th></tr></thead><tbody id='colsdata'></tbody></table>"
      );
    data.map((f) => {
      ...
      me.dialog.set_primary_action(__("Save"), function () {
        ...
        wrapper.find("#colsdata tr").each(function () {
          if (
            ...
            $(this).find("td:eq(1)").find("select").val() == undefined
          ) {
            check_vals = false;
          } else {
...
var add_detail_template = Class.extend({
  ...
  get_page_templates: function () {
    ...
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_detail_page_templates",
      args: {},
      async: false,
      callback: function (r) {
        me.sections = r.message.templates;
