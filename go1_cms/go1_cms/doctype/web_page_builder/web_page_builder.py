...
class WebPageBuilder(WebsiteGenerator):
    ...
    def sync_sections(self):
        if getattr(frappe.flags, "in_builder_sync", False):
            return

        frappe.flags.in_builder_sync = True
        try:
            # Check if we should sync from child tables to JSON (i.e. Desk UI updates)
            sync_from_child_tables = False
            before_save = self.get_doc_before_save()
            
            if before_save:
                if self.draft_layout_json != before_save.draft_layout_json or self.layout_json != before_save.layout_json:
                    sync_from_child_tables = False
                else:
                    old_sections = [r.section for r in before_save.get("web_section") or []]
                    new_sections = [r.section for r in self.get("web_section") or []]
                    if old_sections != new_sections:
                        sync_from_child_tables = True
            else:
                if not self.draft_layout_json and not self.layout_json and self.web_section:
                    sync_from_child_tables = True

            if sync_from_child_tables:
                layout_str = self.draft_layout_json or self.layout_json
                data = None
                if layout_str:
                    try:
                ...
                existing_sections_by_ps = {}
                for s in sections_list:
                    if not isinstance(s, dict):
                        continue
                    sid = s.get("id")
                    if sid:
                        ps_name = frappe.db.get_value("Page Section", {"layout_id": sid}, "name")
                        if not ps_name and frappe.db.exists("Page Section", sid):
                            ps_name = sid
                        if ps_name:
                            existing_sections_by_ps[ps_name] = s
                    tid = s.get("templateId")
                    if tid:
                        existing_sections_by_ps[tid] = s

                new_sections_list = []
                for row in self.web_section:
                    if not row.section:
                        continue
                    
                    sec_json = existing_sections_by_ps.get(row.section)
                    if sec_json:
                        new_sections_list.append(sec_json)
                    else:
                        try:
                            ps_doc = frappe.get_doc("Page Section", row.section)
                            if not ps_doc.layout_id:
                                import random
                                import string
                                rand_id = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
                                ps_doc.layout_id = f"tmpl-{rand_id}"
                                ps_doc.db_set("layout_id", ps_doc.layout_id)
                            
                            is_template = bool(ps_doc.section_title and frappe.db.exists("Section Template", ps_doc.section_title))
                            new_sec = {
                                "id": ps_doc.layout_id,
                                "type": "template" if is_template else "freebuilder" if ps_doc.section_type == "Custom Section" else "static",
                                "source": "db" if is_template else "static",
                                "name": ps_doc.section_name or ps_doc.section_title or row.section_name,
                                "thumbKey": "",
                                "sectionTitle": ps_doc.section_title or row.section_name,
                                "templateId": ps_doc.section_title if is_template else ps_doc.name,
                                "image": "",
                                "content": {},
                                "variant": 0,
                                "styles": {}
                            ...
                            frappe.log_error(f"Error syncing newly added backend section: {str(e)}", "Web Page Builder Sync")


                
                data["sections"] = new_sections_list
                serialized_data = json.dumps(data)
                self.draft_layout_json = serialized_data
                self.layout_json = serialized_data
            ...
            for i, sec in enumerate(sections_list):
                ...
                sec_name = sec.get("name") or sec.get("sectionTitle") or "Section"
                template_id = sec.get("templateId")

                page_section_name = None

                # Look up Page Section by layout_id (unique frontend section ID)
                if sec_id:
                    page_section_name = frappe.db.get_value("Page Section", {"layout_id": sec_id}, "name")
                    if not page_section_name and frappe.db.exists("Page Section", sec_id):
                        page_section_name = sec_id

                if page_section_name:
                    # Update existing Page Section layout JSON and title directly via db.set_value to avoid hooks recursion
                    try:
                        custom_data = json.dumps(sec.get('children') or sec.get('content') or [])
                        update_values = {
                            "section_title": sec_name,
                            "layout_json": json.dumps(sec),
                            "custom_section_data": custom_data
                        }
                        curr_layout_id = frappe.db.get_value("Page Section", page_section_name, "layout_id")
                        if not curr_layout_id and sec_id:
                            update_values["layout_id"] = sec_id
                        frappe.db.set_value("Page Section", page_section_name, update_values, update_modified=True)
                    except Exception as e:
                        frappe.log_error(f"Error updating page section: {str(e)}", "Web Page Builder Sync")
                else:
                    # 1. Handle predefined template sections from DB
                    if sec_type == 'template' and sec_source == 'db' and template_id:
                        try:
                            # Convert Section Template to a new Page Section
                            if frappe.db.exists("Section Template", template_id):
                                ps_doc = convert_template_to_section(template_id, business=self.business)
                                if ps_doc:
                                    page_section_name = ps_doc.name
                                    custom_data = json.dumps(sec.get('children') or sec.get('content') or [])
                                    # Attach layout_id, layout_json and custom_section_data for E2E sync tracking
                                    frappe.db.set_value("Page Section", page_section_name, {
                                        "layout_id": sec_id,
                                        "layout_json": json.dumps(sec),
                                        "custom_section_data": custom_data
                                    }, update_modified=True)
                        except Exception as e:
                            frappe.log_error(f"Error converting template to section: {str(e)}", "Web Page Builder Sync")

                    # 2. Handle static/custom/freebuilder sections
                    if not page_section_name and sec_id:
                        try:
                            ps = frappe.new_doc("Page Section")
                            ps.section_type = "Custom Section" if sec_type in ['freebuilder', 'Custom Section'] else "Static Section"
                            ps.content_type = "Static"
                            ps.section_title = sec_name
                            ps.layout_id = sec_id
                            ps.layout_json = json.dumps(sec)
                            ps.custom_section_data = json.dumps(sec.get('children') or sec.get('content') or [])
                            ps.save(ignore_permissions=True)
                            page_section_name = ps.name
                        except Exception as e:
                            frappe.log_error(f"Error creating custom page section: {str(e)}", "Web Page Builder Sync")

                if page_section_name:
                    ps_meta = frappe.db.get_value("Page Section", page_section_name, ["section_name", "section_title", "section_type", "content_type"], as_dict=True) or {}
                    row_data = {
                        "section": page_section_name,
                        "section_name": ps_meta.get("section_name") or sec_name,
                        "section_title": ps_meta.get("section_title") or sec_name,
                        "section_type": ps_meta.get("section_type") or "Custom Section",
                        "content_type": ps_meta.get("content_type") or "Data",
                        "idx": i + 1
                    }
                    web_secs.append(row_data)
                    mobile_secs.append(row_data)

            self.set("web_section", [])
            for row in web_secs:
                self.append("web_section", row)

            self.set("mobile_section", [])
            for row in mobile_secs:
                self.append("mobile_section", row)
        finally:
    ...
    def validate(self):
        ...
        if self.is_new():
            ...
            db_layout = frappe.db.get_value(self.doctype, self.name, 'layout_json')
            if db_layout != self.layout_json:
                is_layout_updated = True
        
        if is_layout_updated:
            if self.web_section:
    ...
    def on_update(self):
        ...
        if enable_generate_html:
            ...
            if page_template:
                ...
                file_dest = os.path.join(temp_path, (html_page+'.html'))
...
def convert_template_to_section(template, business=None, section_name=None):
	...
	if not doc.content_type:
		doc.content_type = "Static"
	if doc.dynamic_data and not doc.reference_document:
		doc.dynamic_data = 0
...
def get_section_content(section, content_type):
	section_name = section  # preserve original value before overwrite
	section = frappe.db.get_all('Page Section', filters={'name': section_name}, fields=['section_type','name','reference_document','fetch_product','reference_name', 'no_of_records', 'custom_section_data', 'display_data_randomly','dynamic_data', 'is_login_required','allow_update_to_style','menu','section_title','class_name','css_json','is_full_width'])
	...
	# Fallback: newly dropped templates are not yet persisted as Page Sections.
	# Return the Section Template's content so the editor can display it.
	template = frappe.db.get_all('Section Template', filters={'name': section_name}, fields=['name', 'section_type', 'allow_update_to_style', 'is_full_width', 'dynamic_data', 'display_data_randomly', 'is_login_required', 'reference_document', 'reference_name', 'no_of_records', 'custom_section_data'])
	if template:
		template[0].section_title = section_name
		template[0].content = frappe.db.sql('''select field_label, field_key, field_type, content, options, allow_update_to_style, css_properties_list, name, group_name, fields_json,css_json,css_text,image_dimension from `tabSection Content` where parent = %(parent)s and content_type = %(content_type)s and parenttype = "Section Template" order by idx''',{'parent': section_name, 'content_type': content_type}, as_dict=1)
		fonts_list = frappe.db.get_all("CSS Font",fields=['name','font_family'])
		template[0].fonts_list = fonts_list
		return template[0]
...
@frappe.whitelist()
def update_page_section_props(section_name, props):
	if not props:
		return
	try:
		ps_doc = frappe.get_doc('Page Section', section_name)
		modified = False
		for key, val in props.items():
			if key in ['id', 'name', 'type']:
				continue
			
			# Find if there is an existing Section Content record for this key
			found = False
			if ps_doc.content:
				for item in ps_doc.content:
					if item.field_key == key:
						str_val = json.dumps(val) if isinstance(val, (list, dict)) else str(val or '')
						if item.content != str_val:
							item.content = str_val
							modified = True
						found = True
						break
			...
			if not found:
				str_val = json.dumps(val) if isinstance(val, (list, dict)) else str(val or '')
				ps_doc.append('content', {
					'field_key': key,
					'field_label': key.replace('_', ' ').title(),
					'content': str_val,
					'content_type': 'Data',
					'field_type': 'Data' if not isinstance(val, (list, dict)) else 'List'
				})
				modified = True
		
		if 'bgColor' in props:
			if ps_doc.bg_color != props['bgColor']:
				ps_doc.bg_color = props['bgColor']
				modified = True
		if 'bgImage' in props:
			if ps_doc.bg_image != props['bgImage']:
				ps_doc.bg_image = props['bgImage']
				modified = True

		if modified:
			ps_doc.save(ignore_permissions=True)
	except Exception as e:
		frappe.log_error(f"Error updating props for Page Section {section_name}: {str(e)}", "update_page_section_props")
...
def update_section_order(page, sections):
	...
	# Parse incoming sections (supports both list of strings and list of dicts)
	import re as _re
	parsed_sections = []
	for s in sections:
		if isinstance(s, dict):
			sec_id = s.get('id')
			sec_type = s.get('type')
			sec_props = s.get('props') or {}
			# Detect temporary IDs generated by the frontend:
			# Pattern 1: 'temp-s-{timestamp}' (newly added section before first save)
			# Pattern 2: '{pageId}-s{idx}-{timestamp}' (section loaded from localStorage without real backend ID)
			is_temp_id = (
				not sec_id or
				sec_id.startswith('temp-s-') or
				bool(_re.search(r'-s\d+-\d{10,}$', sec_id))
			)
			if is_temp_id:
			...
			parsed_sections.append({
				'id': s,
				'props': {},
				'orig_type': s.replace('NEW:', '') if s.startswith('NEW:') else ''
			})

	# Rebuild the child table in the correct order
	ordered_rows = []
	for i, item_data in enumerate(parsed_sections):
		section_id = item_data['id']
		props = item_data['props']
		
		if section_id.startswith('NEW:'):
			template_name = item_data['orig_type']
			new_section = convert_template_to_section(template_name, business=doc.business)
			if new_section:
				new_row = doc.append('web_section', {
					'section': new_section.name,
					'section_name': new_section.section_title or new_section.section_name or template_name,
					'section_title': new_section.section_title or template_name,
					'section_type': new_section.section_type or '',
					'idx': i + 1
				})
				ordered_rows.append(new_row)
				update_page_section_props(new_section.name, props)
		else:
			existing_row = next((r for r in doc.web_section if r.section == section_id or r.name == section_id), None)
			if existing_row:
				existing_row.idx = i + 1
				ordered_rows.append(existing_row)
			else:
				ps = frappe.db.get_value(
					'Page Section', section_id,
					['name', 'section_name', 'section_title', 'section_type', 'content_type'],
					as_dict=True
				)
				if ps:
					new_row = doc.append('web_section', {
						'section': ps.name,
						'section_name': ps.section_title or ps.section_name or '',
						'section_title': ps.section_title or '',
						'section_type': ps.section_type or '',
						'content_type': ps.content_type or 'Data',
						'idx': i + 1
					})
					ordered_rows.append(new_row)
			update_page_section_props(section_id, props)
	
	doc.web_section = ordered_rows
	
	# Build layout JSON using the updated section records in memory
	updated_sections = []
	for i, item in enumerate(doc.web_section):
		try:
			ps_doc = frappe.get_doc('Page Section', item.section)
			sec_data = ps_doc.run_method('section_data')
			
			props = {}
			if i < len(parsed_sections):
				props = parsed_sections[i].get('props', {})

			updated_sections.append({
				'id': item.section,
				'name': item.section_title or item.section_name or item.section or '',
				'type': item.section_type or '',
				'section': item.section,
				'section_name': item.section_title or item.section_name or item.section or '',
			...
			frappe.log_error(f'Failed to get section_data for {item.section}: {str(e)}', 'update_section_order')
			
	layout_data = { "sections": updated_sections }
	layout_json_str = frappe.as_json(layout_data)
	doc.layout_json = layout_json_str
	doc.draft_layout_json = layout_json_str
