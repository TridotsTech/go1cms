		...
		if self.content:
			for item in self.content:
				if item.field_label and not item.field_key:
					...
					if check_existing:
						item.field_key = item.field_key + '{0}'.format(frappe.generate_hash(length=4))
...
def get_linked_documents(dt, image_option):
	...
	if image_option == 'Child Table':		
		filtered_list = list(filter(lambda x: x.fieldtype == 'Table', document_fields.fields))
	else:
		filtered_list = list(filter(lambda x: x.fieldtype == 'Link', document_fields.fields))
	if filtered_list:
		docs_list = []
		for item in filtered_list:
			docs_meta = frappe.get_meta(item.options, cached=True)
			attach_fields = list(filter(lambda x: x.fieldtype in ['Attach Image', 'Attach'], docs_meta.fields))
			if attach_fields:
				docs_list.append({'document': item.options, 'fields': attach_fields})
		return docs_list
