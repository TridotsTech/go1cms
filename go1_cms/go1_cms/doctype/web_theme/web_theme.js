...
frappe.ui.form.on("Web Theme", {
  ...
  validate(frm) {
    if (cur_frm.doc.text && cur_frm.doc.text.length != 0) {
      ...
      let dub_flage = tempt_tit_arr.filter(
        (item, index) => tempt_tit_arr.indexOf(item) !== index
      );
  ...
  hide_and_set_heading_values(frm) {
    if (
      (cur_frm.doc.heading && cur_frm.doc.heading.length == 0) ||
      !cur_frm.doc.heading
    ) {
      let heading_data = [
        { name1: "Heading 1", type: "h1" },
        { name1: "Heading 2", type: "h2" },
        { name1: "Heading 3", type: "h3" },
        { name1: "Heading 4", type: "h4" },
        { name1: "Heading 5", type: "h5" },
        { name1: "Heading 6", type: "h6" },
      ];
    ...
    if (cur_frm.doc.heading && cur_frm.doc.heading.length >= 6) {
      $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
        .find('[data-fieldname="heading"]')
        .find(".btn.btn-xs.btn-secondary.grid-add-row")
        .css({ display: "none" });
      $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
        .find('[data-fieldname="heading"]')
  ...
  hide_and_set_button_values(frm) {
    if (
      (cur_frm.doc.buttons_table && cur_frm.doc.buttons_table.length == 0) ||
      !cur_frm.doc.buttons_table
    ) {
      let heading_data = [
        { name1: "Primary Button", type: "primary - btn" },
        { name1: "Secondary Button", type: "secondary - btn" },
        { name1: "Tertiary Button", type: "tertiary - btn" },
      ];
      heading_data.map((each_data) => {
    ...
    if (cur_frm.doc.buttons_table && cur_frm.doc.buttons_table.length == 3) {
      $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
        .find('[data-fieldname="buttons_table"]')
        .find(".btn.btn-xs.btn-secondary.grid-add-row")
        .css({ display: "none" });
      $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
        .find('[data-fieldname="buttons_table"]')
  ...
  hide_and_set_text_values(frm) {
    if (
      (cur_frm.doc.text && cur_frm.doc.text.length == 0) ||
      !cur_frm.doc.text
    ) {
      let heading_data = [
        { name1: "Paragraph", type: "p - Paragraph" },
        { name1: "Anchor", type: "a - Anchor" },
        { name1: "Quote", type: "blockquote - Quote" },
        { name1: "Formatted", type: "pre - Formatted" },
        { name1: "List", type: "li - List" },
      ];
      heading_data.map((each_data) => {
  ...
  hide_and_set_global_fonts(frm) {
    if (
      (cur_frm.doc.global_fonts && cur_frm.doc.global_fonts.length == 0) ||
      !cur_frm.doc.global_fonts
    ) {
      let heading_data = [
        { title: "Primary" },
        { title: "Secondary" },
    ...
    if (cur_frm.doc.global_fonts && cur_frm.doc.global_fonts.length == 4) {
      $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
        .find('[data-fieldname="global_fonts"]')
        .find(".btn.btn-xs.btn-secondary.grid-add-row")
        .css({ display: "none" });
      $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
        .find('[data-fieldname="global_fonts"]')
      ...
      if (cur_frm.doc.heading && cur_frm.doc.heading.length > 0) {
        cur_frm.doc.heading.map((h) => {
          ...
          $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
            .find('[data-fieldname="heading"]')
            .find(".rows")
            .find(`[data-name="${h.name}"]`)
            .find(`[data-fieldname="edit_styles"] .field-area`)
            .css({ display: "block" });
        });
      }
  ...
  pick_color_palette(frm) {
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_theme.web_theme.get_color_palette_datas",
      args: {},
      freeze: true,
      callback: (res) => {
  ...
  open_color_palette_popoup(frm, palettes) {
    ...
    palettes.map((each_p) => {
      p_add_html += `<div class="each-palette-div" style="flex:0 0 calc(33.33% - 20px);position:relative;">
        ...
        each_p.name
      }" name="theme_radio" onclick="select_unselect_palette($(this))" style="position:absolute;top:2px;right:43px;">
		</label></div>`;
    });
  ...
  update_default_values(frm, res) {
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_theme.web_theme.get_color_palette_default_datas",
          ...
          cur_frm.palettes_api_default_data.map((api_each_pop) => {
            if (api_each_pop.css_design && api_each_pop.css_design.length > 0) {
              if (btn.type == api_each_pop.type) {
                ...
                let api_css_parse_data = JSON.parse(
                  api_each_pop.css_design
                ).split(";");
                      ...
                      if (css_parse_data && css_parse_data.length > 0) {
                        css_parse_data.map((each_pop, idxx) => {
                          ...
                          if (css_key_val[0] == each_api_prp.split(":")[0]) {
                            css_parse_data[idxx] =
                              each_api_prp.split(":")[0] +
                              ":" +
                              each_api_prp.split(":")[1];
                          }
                        });
                      }
                    });
            ...
            if (api_each_pop.style_json && api_each_pop.style_json.length > 0) {
              if (btn.type == api_each_pop.type) {
                ...
                let api_css_each_ele_parse_data =
                  Object.keys(api_css_parse_data);
                ...
                api_css_each_ele_parse_data.map((res) => {
                  if (css_each_ele_parse_data.includes(res)) {
                    Object.keys(api_css_parse_data).map((each_api_prp) => {
                      if (
                        css_parse_data &&
                        Object.keys(css_parse_data).length > 0
                      ) {
                        Object.keys(css_parse_data).map((each_pop, idxx) => {
                          if (each_pop == each_api_prp && each_pop == res) {
                            css_parse_data[each_pop] =
                              api_css_parse_data[each_api_prp];
                          }
                        });
                      }
                    });
                    ...
                    api_css_parse_data.map((each_api_prp) => {
                      if (css_parse_data && css_parse_data.length > 0) {
                        css_parse_data.map((each_pop, idxx) => {
                          let css_key_val = each_pop.split(":");
                          if (
                            css_key_val[0] == each_api_prp.split(":")[0] &&
                            css_key_val[0] == res
                          ) {
                            css_parse_data[idxx] =
                              each_api_prp.split(":")[0] +
...
frappe.ui.form.on("Typography Heading", {
  form_render(frm, cdt, cdn) {
    ...
    $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
      ...
      .find(".btn.btn-danger.btn-sm.pull-right.grid-delete-row")
      .css({ display: "none" });
    $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
      .find(`[data-name="${item.name}"]`)
      .find(
        ".btn.btn-secondary.btn-sm.pull-right.grid-insert-row-below.hidden-xs"
      )
      .css({ display: "none" });
    $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
      .find(`[data-name="${item.name}"]`)
      .find(".btn.btn-secondary.btn-sm.pull-right.grid-insert-row.hidden-xs")
      .css({ display: "none" });
    $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
      .find(`[data-name="${item.name}"]`)
      .find(".btn.btn-secondary.btn-sm.pull-right.grid-duplicate-row.hidden-xs")
      .css({ display: "none" });
    $(`[data-route="Form/Web Theme/${cur_frm.doc.name}"]`)
      .find(`[data-name="${item.name}"]`)
      .find(".btn.btn-secondary.btn-sm.pull-right.grid-append-row")
      .css({ display: "none" });
  ...
  edit_styles(frm, cdt, cdn) {
    ...
    if (item.type) {
      frappe.call({
        method:
          "go1_cms.go1_cms.doctype.web_theme.web_theme.get_json_render_properties",
        ...
        callback: (res) => {
          ...
          if (res && res.message && res.message.length > 0) {
            ...
            res.message[0].field_key = item.type.replace(
              /[&\/ \\#,+()$~%.'":*?<>{}]/g,
              ""
            );
...
frappe.ui.form.on("Typography Text", {
    ...
    if (item.type) {
      frappe.call({
        ...
        callback: (res) => {
          ...
          if (res && res.message && res.message.length > 0) {
            res.message[0].css_json = item.style_json ? item.style_json : null;
            res.message[0].field_key = item.type
              .replace(/[&\/ \\#,+()$~%.'":*?<>{}]/g, "")
              .split("-")[0];
            open_edit_styles_popup(item, res.message[0]);
          } else {
  ...
  before_text_remove(frm, cdt, cdn) {
    ...
    if (del_item && del_item.name1 && text_data.includes(del_item.name1)) {
      frappe.validated = false;
      frappe.throw(
        `Row <b>${del_item.idx}</b> with value <b>${del_item.name1}</b> can't be delete..!`
      );
    }
  },
});
...
function align_edit_style_btn(frm, item) {
  let btn_wrapper =
    frm.fields_dict[item.parentfield].grid.grid_rows_by_docname[item.name]
      .grid_form.fields_dict["edit_styles"].wrapper;
  $(btn_wrapper).find(".btn.btn-xs.btn-default").css({ "margin-top": "27px" });
  $(btn_wrapper)
    .find(".btn.btn-xs.btn-default")
...
function open_edit_styles_popup(item, res) {
  ...
  if (res.css_properties_list.length > 0) {
          ...
          for (const [key, value] of Object.entries(json_data)) {
            ...
            if (key == child_properties[k].fieldname) {
              ...
              // console.log(key,">> equal <<",value,">> child properties <<",child_properties[k].fieldname)
                ...
                (key == child_properties[k].fieldname && key == "bg-mask-color")
              ) {
        ...
        for (const [key, value] of Object.entries(json_data)) {
          if (key == filelist[i].fieldname) {
            ...
            if (
              ...
              (key == filelist[i].fieldname && key == "bg-gradient-color") ||
              (key == filelist[i].fieldname && key == "bg-pattern-color") ||
    ...
    $(mask_html).insertBefore(
      elementstyledialog.$wrapper.find(
        '[data-fieldname="bg-mask-color"] button'
      )
    );
    ...
    elementstyledialog.set_primary_action(__("Save"), function () {
      ...
      filelist.map((each_list) => {
        if (
          each_list.child_properties &&
          each_list.child_properties.length > 0
        ) {
              ...
              if (ls.variable_name == "--go1-button-$-gc") {
                ...
                if (
                  ...
                  popup_variable_color_codes["bg-gradient-color"]
                    ? (css_design +=
                        ls.variable_name.replace("$", res.field_key) +
                        ":" +
                        popup_variable_color_codes["bg-gradient-color"])
                    : "";
      ...
      for (let k in values) {
        if (values[k] != "" && values[k] != "0px") {
          ...
          if (is_allow_units == 1) {
            units = elementstyledialog.$wrapper
              .find(".form-control[data-fieldname='" + k + "']")
              .parent()
              .find("select.unit")
              .val();
                ...
                if (ls.fieldname == k) {
                  ...
                  if (ls.hasOwnProperty("variable_name")) {
                    ...
                    if (k == "background-image") {
                      ...
                      // }
                      let font_famil_name = fonts_data.find(
                        (o) => o === values[k]
                      );
                      let check_global = global_fonts_data.find(
                        (o) => o === values[k]
                      );
                      if (check_global == undefined || check_global == "") {
                        var css_font_name = css_fonts_list.find(
                          (o) => o.name == values[k]
                        );
            ...
            each_list.child_properties.map((ls) => {
              if (ls.fieldname == k) {
                ...
                if (ls.hasOwnProperty("variable_name")) {
                  if (
                    k == "font-size" &&
                    ls.allow_media_query &&
                    enable_font_media_query
                  ) {
                    Object.keys(fonts_media_query_data).map((e_res) => {
                      if (e_res == "mbl-font-size") {
                        css_design +=
      ...
      if (popup_variable_color_codes.hasOwnProperty("bg-pattern-color")) {
        ...
        gra_patt_mask["bg-pattern-color"]
          ? (style_json["bg-pattern-color"] = gra_patt_mask["bg-pattern-color"])
          : "";
      }
    ...
    for (var i = 0; i < child_sections.length; i++) {
      elementstyledialog.$wrapper
        .find(".frappe-control[data-fieldname='" + child_sections[i] + "']")
        .parent()
        .parent()
        .parent()
        .parent()
        .attr(
          "style",
          "margin-bottom:10px;border-top:none !important;margin-top: -20px;width: calc(100% + 40px);margin-left: -20px;padding-left: 20px;padding-right: 20px;margin-bottom: 0;position: relative;m"
        );
      elementstyledialog.$wrapper
        .find(".frappe-control[data-fieldname='" + child_sections[i] + "']")
        .parent()
        ...
        .attr(
          "style",
          "font-weight: 600;margin-bottom: 20px;margin-top: 0;margin-left: -20px;width: 100%;max-width: calc(100% + 40px);position: absolute;padding-left: 20px;padding-top: 10px;border-bottom: 1px solid var(--border-color);border-top: 1px solid var(--border-color);font-size:14px;"
        );
      elementstyledialog.$wrapper
        .find(".frappe-control[data-fieldname='" + child_sections[i] + "']")
        .parent()
      ...
      if (i == 0) {
        elementstyledialog.$wrapper
          ...
          .attr(
            "style",
            "font-weight: 600;margin-bottom: 20px;margin-top: 4px;margin-left: -20px;width: 100%;max-width: calc(100% + 40px);position: absolute;padding-left: 20px;padding-top: 10px;border-bottom: 1px solid var(--border-color);border-top: 1px solid var(--border-color);font-size:14px;"
          );
      }
    }
    for (var i = 0; i < units_sections.length; i++) {
      ...
      elementstyledialog.$wrapper
        .find(".form-control[data-fieldname='" + units_sections[i] + "']")
        .parent()
        .attr("style", "display: flex;position: relative;max-width: 140px;");
    ...
    if (enable_font_media_query) {
      ...
      $(
        elementstyledialog.$wrapper.find('[data-fieldname="font-size"] input')
      ).on("keyup", (e) => {
        let entered_value = $(e.target).val();
        let unit = $(
          elementstyledialog.$wrapper.find(
            '[data-fieldname="font-size"] select'
          )
        ).val();
        let input_value = $(
          elementstyledialog.$wrapper.find('[data-fieldname="font-size"] input')
      ...
      $(
        elementstyledialog.$wrapper.find('[data-fieldname="font-size"] select')
      ).on("change", (e) => {
    ...
    function generate_colors_html(gradiant_data, bg_type) {
      ...
      gradiant_data.map((each_gradiant) => {
        ...
        if (
          ...
          temp_html = $(`<div class="each-bg-div" bg_type="${bg_type}" value="${
            each_gradiant.css_properties
            ...
            each_gradiant.css_properties
					...
					<img bg_type="${bg_type}" value="${each_gradiant.css_properties}" src=${
          ...
          temp_html.click((e) => {
            ...
            if ($(e.target).attr("bg_type") == "Gradient") {
              ...
              elementstyledialog.$wrapper
                ...
                .css({
                  background: `${$(e.target).attr("value").split(";")[0]}`,
                });
              bg_dialog.hide();
            } else if ($(e.target).attr("bg_type") == "Pattern") {
              popup_variable_color_codes["bg-pattern-color"] = $(e.target).attr(
                "value"
              );
              elementstyledialog.$wrapper
                .find(
              ...
              popup_variable_color_codes["bg-mask-color"] = $(e.target).attr(
                "value"
              );
              elementstyledialog.$wrapper
                .find(
