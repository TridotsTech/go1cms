        ...
        try:
            if self.is_active == 1:
                existing_list = frappe.db.sql(
                    '''UPDATE `tabWeb Theme` SET is_active=0 WHERE name!="{theme_name}"'''.format(theme_name=self.name))
                frappe.db.commit()
    ...
    try:
        ...
        if doc_obj.heading and len(doc_obj.heading) > 0:
            for var in doc_obj.heading:
                if var.css_design:
                    ...
                    g_fonts = replace_value_of_globalfonts(
                        var.css_design, doc_obj)
                    if g_fonts:
        ...
        if doc_obj.text and len(doc_obj.text) > 0:
            for var in doc_obj.text:
                if var.css_design:
                    # frappe.log_error(var.name1,">>tag name<<")
                    global_fonts = replace_value_of_globalfonts(
                        var.css_design, doc_obj)
                    if global_fonts:
            ...
            for btn in doc_obj.buttons_table:
                if btn.css_design:
                    ...
                    if replace_value_of_globalfonts(btn.css_design, doc_obj):
                        btn_css += replace_value_of_globalfonts(
                            btn.css_design, doc_obj)
        ...
        if doc_obj.default_header:
            ...
            if doc_obj.header_settings.font_family:
                doc_obj.header_settings.font_family = frappe.db.get_value(
                    "CSS Font", doc_obj.header_settings.font_family, "font_family")
        ...
        if doc_obj.default_footer:
            doc_obj.footer_css = frappe.get_doc(
                "Footer Component", doc_obj.default_footer)
            """ for map font family """
            if doc_obj.footer_css.font_family:
                doc_obj.footer_css.font_family = frappe.db.get_value(
                    "CSS Font", doc_obj.footer_css.font_family, "font_family")
        ...
        for page in pages:
            ...
            for x in web_sections:
                ...
                sec_columns = frappe.db.get_all("Section Column CSS", filters={
                    "parent": x.name}, fields=['css_text', 'column_class'])
            ...
            if page.edit_header_style == 1:
                if page.is_transparent_header == 1:
                    ...
                    if "/" in p_route:
                        p_route = p_route.split('/')[1]
                    webtheme_css += "."+p_route + \
                        " .go1-cms-header{background:transparent;}"
                    webtheme_css += "."+p_route + \
                        " .go1-cms-menu li a{color:"+page.menu_text_color+";}"
                    if page.menu_hover_bg:
                        webtheme_css += "."+p_route + \
                            " .go1-cms-menu li a:hover{background-color:" + \
                            page.menu_hover_bg+";}"
                    if page.menu_hover_text_color:
                        webtheme_css += "."+p_route + \
                            " .go1-cms-menu li a:hover{color:" + \
                            page.menu_hover_text_color+";}"
                    if header_settings:
                        webtheme_css += "."+p_route + \
                            " .sticky_header .go1-cms-menu li a{color:" + \
                            header_settings.menu_text_color+";}"
                        webtheme_css += "."+p_route + \
                            " .sticky_header .go1-cms-menu li a:hover{color:" + \
                            header_settings.menu_hover_color+";}"
                    else:
                        if is_custom_header:
                            if doc_obj.header_settings:
                                webtheme_css += "."+p_route + \
                                    " .sticky_header .go1-cms-menu li a{color:" + \
                                    doc_obj.header_settings.menu_text_color+";}"
                                webtheme_css += "."+p_route + \
                                    " .sticky_header .go1-cms-menu li a:hover{color:" + \
        ...
        if each_arr and len(each_arr) == 2:
            if (each_arr[1]) == "Primary(Font Family)":
                if doc_obj.primary_font_family:
                    api_css_parse_data[idxx] = each_arr[0] + \
                        ':'+doc_obj.primary_font_family
                else:
...
def get_cms_json():
    try:
        ...
        if result:
            ...
            final_data["cms_data"] = []
        fonts = frappe.db.get_list("CSS Font", fields=[
            "font_family", "font_family_category", "font_name", "font_type", "font_url", "name"])
        if fonts:
        ...
        frappe.log_error(frappe.get_traceback(
        ), "go1_cms.go1_cms.doctype.web_theme.web_theme.get_cms_json")
...
def get_json_render_properties(field_type):
    try:
        css_properties = frappe.db.get_all("Field Types Property", filters={
            "field_type": field_type}, fields=['css_properties_list'])
        ...
        if css_properties:
            ...
            if css_properties[0].css_properties_list and len(css_properties[0].css_properties_list) > 0:
                css_properties[0].css_properties_list = json.loads(
                    css_properties[0].css_properties_list)
...
def make_random_class_name():
    try:
        ...
        frappe.log_error(frappe.get_traceback(
        ), "go1_cms.go1_cms.doctype.web_theme.web_theme.make_random_class_name")


@frappe.whitelist()
def get_color_palette_datas():
    try:
        all_palate_data = frappe.db.get_list("Color Palette", fields=["name", "name1", "image", "body_text_color", "accent_color",
                                                                      "dark_color", "secondary_color", "heading_text_color", "light_color", "primary_color"], filters={"is_active": 1})
...
def get_color_code_data(bg_type):
    try:
        ...
        frappe.log_error(frappe.get_traceback(
        ), "go1_cms.go1_cms.doctype.web_theme.web_theme.def get_color_code_data")
# end
