...
frappe.ui.form.on("Color Palette", {
  ...
  hide_and_set_color_values(frm) {
    if (
      (cur_frm.doc.typo_default_color &&
        cur_frm.doc.typo_default_color.length == 0) ||
      !cur_frm.doc.typo_default_color
    ) {
      let heading_data = [
        { type: "h1" },
    ...
    if (
      cur_frm.doc.typo_default_color &&
      cur_frm.doc.typo_default_color.length >= 13
    ) {
      $(`[data-route="Form/Color Palette/${cur_frm.doc.name}"]`)
        .find('[data-fieldname="typo_default_color"]')
...
function open_edit_styles_popup(item, res) {
  ...
  if (res.css_properties_list.length > 0) {
      ...
      if (filelist[i].child_properties) {
        ...
        for (var k = 0; k < child_properties.length; k++) {
          ...
          for (const [key, value] of Object.entries(json_data)) {
            if (key == child_properties[k].fieldname) {
              ...
              if (
                child_properties[k].fieldname == "color" ||
                child_properties[k].fieldname == "background-color" ||
                child_properties[k].fieldname == "border-color"
              ) {
                child_properties[k].default =
                  check_color_assign_label_name(value);
              }
              // end
            }
          }
          ...
          if (child_properties[k].allow_units) {
            if (child_properties[k].allow_units == 1) {
              ...
              if (
                item.doctype &&
                (item.doctype == "Typography Heading" ||
                  item.doctype == "Typography Text" ||
                  item.doctype == "Web Theme Button") &&
                cur_frm.doc.font_size_with_property &&
                child_properties[k].default &&
                child_properties[k].fieldname == "font-size" &&
                child_properties[k].allow_unit_set
              ) {
                units_sections_with_units.push({
                  field_name: child_properties[k].fieldname,
                  unit_prop: cur_frm.doc.font_size_with_property.split("-")[1],
                });
              }
              // end
            }
        ...
        for (const [key, value] of Object.entries(json_data)) {
          if (key == filelist[i].fieldname) {
            ...
            // compare the color and set default values and override
            if (
              filelist[i].fieldname == "color" ||
              filelist[i].fieldname == "background-color" ||
              filelist[i].fieldname == "border-color"
            ) {
    ...
    elementstyledialog.set_primary_action(__("Save"), function () {
      ...
      for (let k in values) {
        if (
          check_colors_lable_and_assign_fieldname(values[k]) != "" &&
          check_colors_lable_and_assign_fieldname(values[k]) != "0px"
        ) {
                ...
                if (ls.fieldname == k) {
                  ...
                  if (ls.hasOwnProperty("variable_name")) {
                    ...
                    if (k == "background-image") {
                      ...
                      let font_famil_name = res.fonts_list.find(
                        (o) =>
                          o.name ===
                          check_colors_lable_and_assign_fieldname(values[k])
                      );
...
function check_colors_lable_and_assign_fieldname(label_name) {
  if (label_name) {
    if (label_name == "Primary Color") {
      ...
      return cur_frm.doc.heading_text_color
        ? cur_frm.doc.heading_text_color
        : "";
    }
  } else {
    return "";
...
function check_color_assign_label_name(val) {
  if (
    val &&
    (cur_frm.doc.heading_text_color ||
      cur_frm.doc.body_text_color ||
      cur_frm.doc.dark_color ||
      cur_frm.doc.light_color ||
      cur_frm.doc.accent_color ||
      cur_frm.doc.secondary_color ||
      cur_frm.doc.primary_color)
  ) {
    if (
      cur_frm.doc.heading_text_color &&
      val == cur_frm.doc.heading_text_color
    ) {
      return "Heading Text Color";
    } else if (
      cur_frm.doc.body_text_color &&
      val == cur_frm.doc.body_text_color
    ) {
      ...
      cur_frm.doc.secondary_color &&
      val == cur_frm.doc.secondary_color
    ) {
