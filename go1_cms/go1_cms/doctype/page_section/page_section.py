...
class PageSection(Document):
	...
	def section_data(self, customer=None, add_info=None,store_business=None):
		...
		json_obj['image'] = self.image
		json_obj['bg_image'] = self.bg_image
		json_obj['bg_color'] = self.bg_color
		json_obj['bg_type'] = self.bg_type
		json_obj['title'] = self.title
		json_obj['sub_title'] = self.sub_title
		json_obj['custom_css'] = self.custom_css
		json_obj['custom_js'] = self.custom_js
		businesss = ''
		...
		if self.section_type == 'Predefined Section' and not self.is_login_required:
			...
			# Map lists and map testimonials fields for frontend template compatibility
			if json_obj.get('data') and isinstance(json_obj['data'], list):
				mapped_data = []
				for item in json_obj['data']:
					if isinstance(item, dict):
						new_item = item.copy()
						# Preserve the database primary key for backend updates
						if 'docName' not in new_item:
							new_item['docName'] = item.get('name')
						
						# For Testimonial: map posted_by to name and title so templates render correctly
						if self.reference_document == 'Testimonial':
							name_val = item.get('posted_by') or item.get('name1') or 'User Name'
							new_item['name'] = name_val
							new_item['title'] = name_val
						
						mapped_data.append(new_item)
					else:
						mapped_data.append(item)
				
				json_obj['data'] = mapped_data
				json_obj['list_items'] = mapped_data
				json_obj['testimonial_items'] = mapped_data
