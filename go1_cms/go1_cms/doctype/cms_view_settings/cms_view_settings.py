...
class CMSViewSettings(Document):
    pass
...
def update(view):
    ...
    doc = frappe.get_doc("CMS View Settings", view.name)
    doc.label = view.label
    doc.icon = view.icon
    doc.route_name = view.route_name or ""
    doc.load_default_columns = view.load_default_columns or False
...
@frappe.whitelist()
def delete(name):
    if frappe.db.exists("CMS View Settings", name):
        frappe.delete_doc("CMS View Settings", name)
...
def public(name, value):
    ...
    doc = frappe.get_doc("CMS View Settings", name)
    doc.public = value
