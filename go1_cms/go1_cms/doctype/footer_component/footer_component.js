  ...
  open_footer_popoup(frm, layouts) {
    ...
    layouts.map((each_p) => {
      p_add_html += `<div class="each-palette-div" style="flex:0 0 calc(25% - 20px);position:relative;">
        ...
        each_p.name
      }" name="theme_radio" onclick="select_unselect_layout($(this))" style="position:absolute;top:0px;left:20px;">
		</label></div>`;
    });
...
var add_new_section = Class.extend({
  ...
  get_section_templates: function () {
    ...
    frappe.call({
      method:
        "go1_cms.go1_cms.doctype.web_page_builder.web_page_builder.get_footer_section_templates",
      args: {
        device_type: me.device_type,
      },
      async: false,
    ...
    if (results.length > 0 || me.list_section_data.length > 0) {
      frappe.call({
        ...
        args: {
          ...
          section: me.section,
          business: cur_frm.doc.business,
          template: me.fields.section_title,
        },
        freeze: true,
        callback: function (r) {
          if (r.message.status == "Success") {
            ...
            // me.dialog.hide();
            cur_frm.set_value("title", "###");
            cur_frm.set_value("title", cur_frm.doc.title);
            cur_frm.save();
          }
        },
      });
