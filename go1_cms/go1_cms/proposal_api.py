...
def get_website_context(context):
    ...
    if social_tracking_codes.google_site_verification:
        site_verification = frappe.render_template('{{metacode}}', {
                                                   'metacode': h.unescape(social_tracking_codes.google_site_verification)})
    else:
        site_verification = ""
    ...
    if layout_settings.header_template:
        default_header_template = frappe.db.get_value(
            "Header Template", layout_settings.header_template, "route")
    else:
        default_header_template = "templates/layout/basenavbar.html"
    if layout_settings.footer_template:
        default_footer_template = frappe.db.get_value(
            "Footer Template", layout_settings.footer_template, "route")
    else:
...
def get_pagebuilder_context(context, doc=None):
    ...
    context.path = path
    if doc:
        page_builder = frappe.get_all("Page Builder", fields=["name"], filters={
                                      "name": doc.name, "is_active": 1})
    else:
        ...
        if page_route and page_route != "/":
            page_builder = frappe.get_all("Page Builder", fields=["name"], filters={
                                          "name": page_route, "is_active": 1})
        else:
            page_builder = frappe.get_all("Page Builder", fields=["name"], filters={
                                          "name": "homepage", "is_active": 1})
    if page_builder:
        page_component = frappe.get_all("Page Component", fields=['component'], filters={
                                        "parent": page_builder[0].name}, order_by="idx asc")
        if page_component:
            for item in page_component:
                dynamic_list = frappe.get_all("Section", fields=[
                                              "*"], filters={"name": item.component, "content_type": "Dynamic", "generate_as_jinja": 1})
                for section in dynamic_list:
                    if section.section_template and section.content_type == "Dynamic":
                        content = frappe.get_list("Page Content", fields=[
                                                  'content_key', 'content'], filters={"parent": section.section_template})
                        for cont in content:
                    ...
                    if section.ref_doc_fields:
                        ...
                        fields_list = ', '.join(
                            ['' + str(i) + '' for i in pages_list])
                    else:
                        fields_list = "*"

                    groupby_con = ""
                    if section.get('group_by_fields'):
                        groupby_con = "group by " + \
                            groupby_condition(section.group_by_fields)

                    orderby_con = ""
                    if section.order_by_fields:
                        orderby_con = orderby_condition(
                            section.order_by_fields, section.orderby_type)
                    else:
                    ...
                    if section.number_of_list:
                        ...
                        limit = 3
                    condition = convert_json_conditions(
                        filters, section.reference_doctype, condition)
                    frappe.log_error(condition, "---condition---")
                    query = '''select {fields_list} from `tab{doctype}` where `tab{doctype}`.name != "" {condition} {groupby_con} order by {orderby_con} limit {limit}'''.format(
                        fields_list=fields_list, doctype=section.reference_doctype, condition=condition, groupby_con=groupby_con, orderby_con=orderby_con, limit=limit)
                ...
                static_list = frappe.get_all("Section", fields=["section_type, section_template, content_type, reference_doctype, number_of_list, generate_as_jinja, context_name, filters_json, ref_doc_fields"], filters={
                                             "name": item.component, "content_type": "Static", "generate_as_jinja": 1})

                for s_section in static_list:
                    if s_section.section_template and s_section.content_type == "Static":

                        contents = frappe.get_list("Page Content", fields=[
                                                   'content_key', 'content'], filters={"parent": s_section.section_template})
...
def update_context(context):
    ...
    h = HTMLParser()
    script_codes = []
    default_columns = ['owner', 'name', 'modified_by', '_meta', 'creation', 'doctype',
                       'modified', 'idx', 'docstatus', 'facebook_page_id', 'google_site_verification']
    ...
    google_map_settings = frappe.get_doc(
    ...
    if pageroute:
        ...
        page_route = word[0]
    if page_route:
        seo = frappe.db.get_all('Page SEO', filters={'page_route': page_route}, fields=[
        ...
        seo = frappe.db.get_all('Page SEO', filters={'page_route': "homepage"}, fields=[
                                'meta_title', 'meta_description', 'meta_keywords'])
        if seo:
            context.meta_title = seo[0].meta_title
            context.meta_description = seo[0].meta_description
    ...
    try:
        if filters:
            for f in filters:
                ...
                if fl.operator:
                    cond = prepare_filter_condition(
                        doctype, f, ignore_ifnull=True)
                    if cond:
...
def get_template_details(template):
    try:
        if template:
            templates = frappe.db.get_all("Page Section", fields=[
                                          "*"], filters={"name": template})[0]
            keys = frappe.db.get_all("Page Content", fields=[
                                     "name", "idx", "field_label", "content_key", "field_type", "select_type", "content"], filters={"parent": template}, order_by="idx asc")
    ...
    if cur_domain:
        if not os.path.exists(os.path.join(url, 'templates/pages', cur_domain)):
            frappe.create_folder(os.path.join(
                url, 'templates/pages', cur_domain))
...
def get_template_folder(url, temp=0):
    try:
        ...
        temp_path = ""
        cur_domain = frappe.local.request.host_url.split('/')[-2:][0]
        tem_folder = frappe.db.get_value(
            "Site Domain", cur_domain, "site_name")
...
def update_proposal_data(viewcount, name, customerip=None):
    ...
    if customerip:
        allow = frappe.db.get_value("Viewed Customer Detail", {
                                    "parent": name, "customer_ip": customerip})

        if not allow:
            total = frappe.db.get_all("Viewed Customer Detail", filters={
                                      "parent": name}, fields=["name"])
