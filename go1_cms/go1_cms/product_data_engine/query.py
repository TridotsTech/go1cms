...
class ProductQuery:
    ...
    def query_items(self, start=0):
        ...
        for condition in self.filters:
            ...
            if field not in m_fields:
                m_fields.append(field)
                n_filters.append(condition)
        m_fields = []
        for condition in self.or_filters:
            field, operator, value = condition
            if field not in m_fields:
                m_fields.append(field)
                n_or_filters.append(condition)

        WebsiteItem = DocType("Website Item")
        query = frappe.qb.from_(WebsiteItem).select(Count("*"))
        mq = convert_filter_to_query(
            filters=n_filters, or_filters=n_or_filters)
        if mq:
...
def convert_filter_to_query(doctype='Website Item', filters=[], or_filters=[]):
    query_filters = None
    query_or_filters = None
    MDoctype = DocType(doctype)
    fields_exists = []
    for condition in filters:
        field, operator, value = condition
        if field in fields_exists:
            continue
        else:
            fields_exists.append(field)

        if query_filters:
            if operator == 'like':
                query_filters = query_filters & (MDoctype[field].like(value))
            elif operator == '=':
                query_filters = query_filters & (MDoctype[field] == value)
            elif operator == 'in':
                query_filters = query_filters & (MDoctype[field].isin(value))
        else:
            if operator == 'like':
                query_filters = (MDoctype[field].like(value))
            elif operator == '=':
                query_filters = (MDoctype[field] == value)
            elif operator == 'in':
                query_filters = (MDoctype[field].isin(value))
    ...
    for condition in or_filters:
        ...
        if query_or_filters:
            if operator == 'like':
                query_or_filters = query_or_filters | (
                    MDoctype[field].like(value))
            elif operator == '=':
                query_or_filters = query_or_filters | (
                    MDoctype[field] == value)
            elif operator == 'in':
                query_or_filters = query_or_filters | (
                    MDoctype[field].isin(value))
        else:
            if operator == 'like':
                query_or_filters = (MDoctype[field].like(value))
            elif operator == '=':
                query_or_filters = (MDoctype[field] == value)
            elif operator == 'in':
                query_or_filters = (MDoctype[field].isin(value))
    m_query = None
    if query_filters and query_or_filters:
        m_query = query_filters & query_or_filters
    elif query_filters:
        m_query = query_filters
    elif query_or_filters:
        m_query = query_or_filters
