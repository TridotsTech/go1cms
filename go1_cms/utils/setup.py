...
from go1_cms.utils.google_maps import get_geolocation
...
def get_settings_from_domain(dt, domain=None, business=None):
	...
	if business:
		...
		reference_doc = DocType(dt)
		query = (
			frappe.qb.from_(reference_doc)
			.select(reference_doc.name)
			.where(reference_doc.business == business)  
		)
	...
	query = (
		frappe.qb.from_(reference_doc)
		.select(reference_doc.name)
		.where((reference_doc.business.isnull() | (reference_doc.business == "")))  
	)
...
def get_settings_value_from_domain(dt, field, domain=None, business=None):
	...
	if business:
		...
		reference_doc = DocType(dt)
		query = frappe.qb.from_(reference_doc)
		fields = ['name'] + field.split(',') if field else ['name']  
	...
	if field:
		additional_fields = field.split(',') 
		query = query.select(*additional_fields) 

	query = query.where((reference_doc.business.isnull() | (reference_doc.business == "")))
...
def get_business_from_web_domain(domain, raise_exception=False):
	...
	check_website = frappe.db.get_all('Site Settings', filters={'domain_name': domain}, fields=['business'])
	if check_website:
...
def send_indexing_request(doc, operation_type='URL_UPDATED'):
	try:
		...
		if frappe.db.get_value("Web Theme", settings, "enable_google_indexing"):
			...
			from go1_cms.utils.google_indexing import publish_site
			publish_site(url=url, operation_type=operation_type)
			# frappe.enqueue('go1_cms.utils.google_indexing.publish_site', \
...
def get_favicon_for_domain(dt, domain=None, business=None):
	if not business:
		...
		if domain:
			business = get_business_from_web_domain(domain)
	reference_doc = DocType(dt)
	query = frappe.qb.from_(reference_doc).select(reference_doc.name)
	if business:
		business = check_parent_business(business) 
		query = query.where(reference_doc.business == business)  
...
@frappe.whitelist()
def clear_api_log():
	frappe.enqueue('go1_cms.utils.setup.delete_api_logs')

@frappe.whitelist()
def clear_executed_command_log():
	frappe.enqueue('go1_cms.utils.setup.delete_executed_command_logs')
...
def delete_api_logs():
	try:
		enable_api_log = frappe.db.get_single_value('Common Settings', 'enable_api_logs')
		if enable_api_log:
			api_log_doc = DocType('API Log')
			api_log_group_doc = DocType('Api Log Group')

			logs_query = (
				frappe.qb.from_(api_log_doc)
				.select(frappe.qb.functions.group_concat(frappe.qb.functions.concat('"', api_log_doc.name, '"')))
				.where(api_log_doc.creation < frappe.qb.functions.date_sub(frappe.qb.functions.curdate(), interval=7))
				.orderby(api_log_doc.creation)
				.limit(100)
			)
			...
			if logs and logs[0] and logs[0][0]:
				...
				delt = (frappe.qb.delete(api_log_group_doc).where(api_log_group_doc.parent.isin(log_names)).run())
				
				dlt =(frappe.qb.delete(api_log_doc).where(api_log_doc.name.isin(log_names)).run())
			
	except Exception as e:
		frappe.log_error(frappe.get_traceback(), 'go1_cms.utils.setup.delete_api_logs')

@frappe.whitelist()
def delete_executed_command_logs():
	try:
		executed_command_doc = DocType('Executed Command')
		logs_query = (
			frappe.qb.from_(executed_command_doc)
			.select(frappe.qb.functions.group_concat(frappe.qb.functions.concat('"', executed_command_doc.name, '"')))
			.where(executed_command_doc.creation < frappe.qb.functions.date_sub(frappe.qb.functions.curdate(), interval=30))
			.orderby(executed_command_doc.creation)
			.limit(100)
		)
		...
		if logs and logs[0] and logs[0][0]:
			log_names = logs[0][0]
			dlt =(frappe.qb.delete(executed_command_doc).where(executed_command_doc.name.isin(log_names)).run())
		
	except Exception as e:
		frappe.log_error(frappe.get_traceback(), 'go1_cms.utils.setup.delete_executed_command_logs')
...
def get_theme_settings(domain=None, business=None):
	...
	check_website = frappe.db.get_all('Site Settings', filters=filters, fields=['business', 'theme', 'home_page', 'domain_name'])
...
def update_business_default(parent, key, value):
	default_value_doc = DocType('DefaultValue')
	update_query = (
		frappe.qb.update(default_value_doc)
		.set(default_value_doc.defvalue, value)
		.where(default_value_doc.parent == parent)
		.where(default_value_doc.defkey == key)
	)
	update_query.run()
...
def update_help_articles():
	...
	module_def_doc = DocType('Module Def')
	modules_query = (
		frappe.qb.from_(module_def_doc)
		.select('*')
		.where(module_def_doc.app_name.isin(apps))
	)
	modules_list = modules_query.run(as_dict=True)
				...
				if out:
					...
					for k in keys:
						for item in out[k]:
							...
							doctype_name = k  
							help_article_doc = DocType('Help Article')

							check_record_query = (
								frappe.qb.from_(help_article_doc)
								.select(help_article_doc.name)
								.where(help_article_doc.category == category)
								.where(help_article_doc.doctype_name == doctype_name)
							)
							check_record = check_record_query.run(as_dict=True)
							if check_record:
