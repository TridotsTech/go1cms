...
def get_settings_from_domain(dt, domain=None, business=None):
    ...
    if business:
        ...
        settings = frappe.db.sql(
            '''select name from `tab{0}` {1}'''.format(dt, condition), as_dict=1)
        if settings:
            return get_settings(dt, settings[0].name)
    settings = frappe.db.sql(
        '''select name from `tab{0}` where business is null or business = ""'''.format(dt), as_dict=1)
    if settings:
        return get_settings(dt, settings[0].name)
...
def get_settings(dt, dn):
    ...
    if dt_map.get(dt):
        path = get_files_path()
        file_path = os.path.join(path, 'settings', dt_map.get(
            dt), '{0}.json'.format(dn.lower()))
...
def get_settings_value_from_domain(dt, field, domain=None, business=None):
    ...
    if business:
        ...
        settings = frappe.db.sql('''select name, {2} from `tab{0}` {1}'''.format(
            dt, condition, field), as_dict=1)
        if settings:
            return settings[0][field]

    settings = frappe.db.sql(
        '''select name, {1} from `tab{0}` where business is null or business = ""'''.format(dt, field), as_dict=1)
    if settings:
        return settings[0][field]
...
def get_business_from_web_domain(domain, raise_exception=False):
    ...
    check_website = frappe.db.get_all(
        'Website', filters={'domain_name': domain}, fields=['business'])
    if check_website:
        if check_website[0].business:
            state = frappe.db.get_value(
                "Business", check_website[0].business, "disable")
...
def get_website_user_home_page(user):
    ...
    if domain:
        ...
        website = frappe.db.get_all(
            'Website', filters={'domain_name': domain}, fields=['home_page'])
...
def send_indexing_request(doc, operation_type='URL_UPDATED'):
    try:
        ...
        ), 'ecommerce_business_store.ecommerce_business_store.setup.send_indexing_request')
...
def check_parent_business(business):
    try:
        is_group, parent = frappe.db.get_value(
            'Business', business, ['is_group', 'parent_business'])
...
def write_file(doc):
    ...
    if doc.attached_to_doctype and doc.attached_to_name:
        ...
        try:
            business = frappe.db.get_value(
                doc.attached_to_doctype, doc.attached_to_name, 'business')
        except Exception as e:
            try:
                business = frappe.db.get_value(
                    'Product', doc.attached_to_name, 'restaurant')
            except Exception as e:
                pass
    if not business:
        shop_user = frappe.db.get_all('Shop User', filters={
                                      'name': frappe.session.user}, fields=['name', 'restaurant'])
    ...
    if folder:
        ...
        check_folder = frappe.db.get_all(
            'File', filters={'file_name': folder, 'folder': 'Home'})
        if not check_folder:
    ...
    if not folder:
        ...
        if doc.is_private:
            doc.file_url = "/private/files/{0}/{1}".format(
                folder, doc.file_name)
        else:
...
def get_favicon_for_domain(dt, domain=None, business=None):
    ...
    if settings:
        ...
        if seting.theme:
            ...
            if frappe.db.get_value("Web Theme", {"is_active": 1}, 'favicon'):
                favicon = frappe.db.get_value(
                    "Web Theme", {"is_active": 1}, 'favicon')
    ...
    if settings:
        ...
        if seting.theme:
            if seting.theme:
                favicon = frappe.db.get_value(
                    "Web Theme", seting.theme, 'favicon')
            else:
                if frappe.db.get_value("Web Theme", {"is_active": 1}, 'favicon'):
                    favicon = frappe.db.get_value(
                        "Web Theme", {"is_active": 1}, 'favicon')
...
def get_query_condition_email_group_member(user):
    ...
    if "Vendor" in frappe.get_roles(user):
        shop_user = frappe.db.get_all(
            'Shop User', filters={'name': user}, fields=['restaurant'])
        if shop_user and shop_user[0].restaurant:
...
def update_api_log(cmd):
    ...
    if common_settings.enable_api_logs:
        if not common_settings.api_log_method or len(common_settings.api_log_method) == 0:
            return
        check_method = next(
            (x for x in common_settings.api_log_method if x.method == cmd), None)
        if check_method:
            ...
            get_user = frappe.db.get_all(
                'API Log', filters={'user': cur_user}, fields=['name'])
            if get_user:
                doc = frappe.get_doc({
                ...
                headers = json.dumps({'host': frappe.request.headers.get('host'), 'user_agent': frappe.request.headers.get(
                    'user_agent'), 'origin': frappe.request.headers.get('origin')})
                parameters = frappe.as_json(frappe.form_dict)
                doc = frappe.new_doc('API Log')
                doc.user = frappe.session.user if (
                    frappe.session.user != 'Guest') else None
                doc.append("log_group", ({"method": cmd, "date": datetime.now(
...
def clear_executed_command_log():
    frappe.enqueue(
        'ecommerce_business_store.utils.setup.delete_executed_command_logs')


@frappe.whitelist()
def delete_api_logs():
    try:
        enable_api_log = frappe.db.get_single_value(
            'Common Settings', 'enable_api_logs')
        if enable_api_log:
            logs = frappe.db.sql(
                '''select group_concat(concat('"', name, '"')) from `tabAPI Log` where creation < date_sub(curdate(), interval 7 day) order by creation limit 100''')
            if logs and logs[0] and logs[0][0]:
                frappe.db.sql('''delete from `tabApi Log Group` where parent in ({name})'''.format(
                    name=logs[0][0]))
                frappe.db.sql(
                    '''delete from `tabAPI Log` where name in ({name})'''.format(name=logs[0][0]))
    except Exception as e:
        frappe.log_error(frappe.get_traceback(),
                         'ecommerce_business_store.utils.setup.delete_api_logs')
...
def delete_executed_command_logs():
    try:
        logs = frappe.db.sql(
            '''select group_concat(concat('"', name, '"')) from `tabExecuted Command` where creation < date_sub(curdate(), interval 30 day) order by creation limit 100''')
        if logs and logs[0] and logs[0][0]:
            frappe.db.sql('''delete from `tabExecuted Command` where name in ({name})'''.format(
                name=logs[0][0]))
    except Exception as e:
        frappe.log_error(frappe.get_traceback(
        ), 'ecommerce_business_store.utils.setup.delete_executed_command_logs')
...
def get_theme_settings(domain=None, business=None):
    ...
    check_website = frappe.db.get_all('Website', filters=filters, fields=[
                                      'business', 'theme', 'home_page', 'domain_name'])
...
def before_insert_all(doc, method):
    ...
    if business and state and doc.doctype != "Business":
        ...
        if int(limit) > 0 and int(count) > int(limit):
            frappe.throw("Can't create new {0}. Maximum {1} count is {3}.".format(
                doc.doctype, doc.doctype, limit))
...
def update_business_default(parent, key, value):
    frappe.db.sql(
        '''update `tabDefaultValue` set defvalue = %s where parent = %s and defkey = %s''', (value, parent, key))
...
def validate_zip_code(zipcode, domain=None, business=None):
    settings = get_settings_from_domain(
        'Order Settings', domain=domain, business=business)
    if settings:
        if int(settings.min_pincode_length) > 0 and len(zipcode) < int(settings.min_pincode_length):
            frappe.throw(frappe._(
                'Postal/Zip code must contain {0} characters').format(settings.min_pincode_length))
        if int(settings.max_pincode_length) > 0 and len(zipcode) > int(settings.max_pincode_length):
            frappe.throw(frappe._(
                'Postal/Zip code should not exceed {0} characters').format(settings.max_pincode_length))
        if settings.pincode == 'Only Number':
            res = re.match('^[0-9]*$', zipcode)
            if not res:
                frappe.throw(
                    frappe._('Postal/Zip code must contain only number'))
        if settings.pincode == 'Number, Letters':
            res = re.search('(?=.*[a-z])(?=.*\d)[a-z\d]', zipcode)
            if not res:
                frappe.throw(
                    frappe._('Postal/Zip code must contain numbers & letters'))
...
def update_help_articles():
    ...
    modules_list = frappe.db.sql(''' select * from `tabModule Def` where app_name in ({}) '''.format(
        ','.join(['"' + x + '"' for x in apps])), as_dict=True)
        ...
        if os.path.exists(file_path):
            with open(file_path, 'r') as f:
                ...
                if out:
                    ...
                    for k in keys:
                        for item in out[k]:
                            check_record = frappe.db.sql('''select name from `tabHelp Article` where category = %(category)s and doctype_name = %(doctype)s''', {
                                                         'category': item.get('category'), 'doctype': k}, as_dict=1)
                            if check_record:
                                doc = frappe.get_doc(
                                    'Help Article', check_record[0].name)
                            else:
...
def clear_logs():
    ...
    for files in filelist:
        commands.append(
            "find . -name '{files}' -exec rm -f {} \;".format(files=files))
