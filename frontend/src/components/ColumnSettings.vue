...
const columns = computed({
  get: () => {
    let _columns = list.value?.data?.columns.filter(
      (el) => el.key != 'action_button'
    )
...
function apply(reload = false, isDefault = false, reset = false) {
  let _columns = reset ? oldValues.value.columns : columns.value
  let _rows = reset ? oldValues.value.rows : rows.value
