...
export const notificationsStore = defineStore('cms-notifications', () => {
  let visible = ref(false)

  const notifications = createResource({
    url: 'go1_cms.api.notifications.get_notifications',
    initialData: [],
    auto: true,
  })

  const mark_as_read = createResource({
    url: 'go1_cms.api.notifications.mark_as_read',
    auto: false,
    onSuccess: () => {
      mark_as_read.params = {}
